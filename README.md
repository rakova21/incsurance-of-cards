[api-docs.json](https://github.com/user-attachments/files/24241025/api-docs.json)# **Программное средство страхования рисков, связанных с использованием банковских платежных карточек и электронных кошельков для физических лиц**
...
<img width="515" height="729" alt="image" src="https://github.com/user-attachments/assets/ec349f3d-e1c1-466e-903b-bddf57855464" />


Программное средство страхования рисков, связанных с использованием банковских платежных карточек и электронных кошельков для физических лиц FinShield обеспечивает взаимодействие между четырьмя основными категориями пользователей: страхователями, страховыми агентами, специалистами по договорам и управляющим. Каждый из участников решает собственные задачи: страхователь получает доступ к информации, оформляет договоры и оплачивает их; страховой агент помогает клиентам с заполнением и формированием договоров; специалист по договорам проверяет их корректность и подтверждает актуальный статус; управляющий управляет пользователями и настройками системы.
...
<img width="974" height="520" alt="image" src="https://github.com/user-attachments/assets/8d22b330-d99d-4e2c-bd42-e82201c31bcd" />

---

<img width="628" height="612" alt="image" src="https://github.com/user-attachments/assets/69b7a338-ceaf-488a-ba70-5ec58282ec88" />

---

<img width="632" height="524" alt="image" src="https://github.com/user-attachments/assets/ac74af35-28e5-41c3-934d-109ddd668f0f" />

---

<img width="514" height="611" alt="image" src="https://github.com/user-attachments/assets/78549801-6802-4092-8bc0-09ce96b5255f" />

---

<img width="456" height="558" alt="image" src="https://github.com/user-attachments/assets/ef9581c1-7ee1-4b6e-a1f4-e9be7dc5722c" />

---

<img width="518" height="909" alt="image" src="https://github.com/user-attachments/assets/7b4e8b9a-6ec8-4051-9391-901394f60449" />
...
<img width="784" height="719" alt="image" src="https://github.com/user-attachments/assets/9494454b-f74e-4d32-9d34-b97aac2b1427" />


Эта схема показывает, что может делать каждый из четырех типов пользователей (Актеров) в системе страхования рисков:
1	Страхователь (Клиент). Может смотреть общую информацию и виды страховок. Главное, он может управлять своими данными (картами, личной информацией), подавать заявки на страхование и прикреплять чеки об оплате.
2	Специалист по договорам. Это главный оператор. Он обрабатывает заявки, подтверждает оплату, смотрит заключенные договоры и проходит внутреннее обучение (аттестацию, опрос).
3	Страховой агент. Его задача – работать с клиентами. Он смотрит информацию, типы страхования и вносит данные клиента для оформления страховки.
4	Управляющий. Он управляет пользователями (дает им доступ), настраивает типы страхования и смотрит аналитику (отчеты) по работе системы.
В целом, система четко разделяет, кто за что отвечает, и охватывает все этапы: от подачи заявки клиентом до административного контроля.
...
Документация к разработанному API представляет собой свод инфор-мации, описывающий функциональность и использование интерфейса про-граммирования приложений. Она служит руководством для разработчиков, позволяя им понимать, как взаимодействовать с API, интегрировать его в свои приложения и использовать предоставляемые им возможности.
Документация для REST API, созданная с использованием инструмента Swagger, играет ключевую роль в упрощении взаимодействия между разра-ботчиками и сервисами. Данный подход позволяет автоматически формиро-вать полное описание всех доступных эндпоинтов, включая их параметры, форматы запросов и ответов, а также коды статусов, которые могут быть воз-вращены в процессе работы с API.
Использование Swagger не только обеспечивает наглядное представле-ние структуры API, но и делает процесс тестирования значительно более удобным. Интерактивная природа документации позволяет разработчикам и пользователям легко взаимодействовать с API, а также быстро проверять и отлаживать запросы. Кроме того, Swagger позволяет экспортировать специ-фикацию, что облегчает интеграцию с такими внешними платформами, как Postman, а также с другими инструментами для разработки и аналитики API.
Благодаря этой системе, документация становится не просто статичным набором данных, а интерактивным и динамичным ресурсом, который способ-ствует пониманию и быстрому усвоению информации о возможностях API. Это делает взаимодействие с сервисом прозрачным и воспроизводимым, что в свою очередь значительно облегчает процесс интеграции и дальнейшего со-провождения программного продукта. Использование Swagger, таким обра-зом, становится важным элементом успешного и эффективного развития про-граммного обеспечения.
На рисунке 1 представлен фрагмент Swagger UI с документацией к про-граммному средству страхования рисков, связанных с использованием бан-ковских платежных карточек и электронных кошельков.

 <img width="974" height="554" alt="image" src="https://github.com/user-attachments/assets/991c1c9e-1996-4eb9-a5f1-179ef10c294d" />


Рисунок 1 – Фрагмент Swagger UI с документацией к программному средству

Swagger автоматически создает спецификацию API в формате OpenAPI, представляемом в виде файлов JSON или YAML, который содержит исчерпы-вающее описание всех эндпоинтов, методов, параметров запросов и ожидае-мых ответов. Эта спецификация может быть сохранена в отдельный файл и затем использована для импорта в сторонние инструменты, такие как Postman. Это значительно упрощает работу команд разработки и тестирова-ния, позволяя быстро формировать коллекции запросов к API и избегать необходимости вручную прописывать все маршруты и параметры, что может быть утомительным и подвержено ошибкам.
При импорте документации из Swagger в Postman все методы API ста-новятся доступными в виде уже подготовленных запросов, что позволяет раз-работчикам и тестировщикам сразу начать тестирование. Каждому запросу сопоставляются соответствующие структуры данных и коды ответов, что зна-чительно упрощает процесс верификации функциональности API. Кроме того, спецификация OpenAPI может включать детальное описание потенциальных ошибок, методы авторизации и другие важные метаданные. Это создаёт воз-можность для моментальной проверки различных сценариев использования API, что ускоряет интеграцию с внешними сервисами и системами.
Таким образом, автоматическое формирование документации не только обеспечивает прозрачность работы с API, но и значительно повышает общую эффективность разработки и тестирования. Наличие структурированного файла спецификации позволяет командам быстрее находить и устранять ошибки, а также успешно интегрировать API в более крупные системы. На рисунке 2 представлен пример созданного JSON-файла, который может быть использован для последующего импорта в Postman, иллюстрируя удобство и функциональность этого подхода.
На рисунке 2 представлен созданный JSON-файл для импорта в Postman.

 <img width="529" height="765" alt="image" src="https://github.com/user-attachments/assets/87f8e5e3-9a9e-4304-85ca-35d4ec45a3dc" />


Рисунок 2 – Созданный JSON-файл для импорта в Postman

[Uploading api-docs.json…]()


	В результате работы была создана документация к API.


### Оценка качества кода

В разделе, посвященном оценке качества кода, будут рассмотрены ключевые метрики, позволяющие глубже понять структуру и характеристики разработанного программного средства. Эти метрики помогут оценить не только сложность и тестируемость кода, но и его поддержку и возможность для дальнейшего развития. Важность этих показателей неоспорима, так как они напрямую влияют на эффективность работы команды разработчиков и стабильность всего проекта. 
Ключевые метрики:
1	Цикломатическая. Измеряет количество линейно независимых путей через программу, что позволяет оценить сложность и потенциальную трудо-емкость тестирования.
2	Покрытие кода тестами. Определяет процент кода, который покрыт автоматическими тестами, что помогает выявлять не протестированные участки.
3	Поддерживаемость. Оценивает, насколько легко вносить изменения в код, включая понимание, изменение и расширение функциональности.
4	Степень связности. Измеряет степень зависимости между модулями, отражая, насколько изменения в одном модуле могут повлиять на другие.
5	Избыточность. Оценивает количество дублирующегося кода в проек-те, что может затруднить поддержку и сопровождение.
Цикломатическая сложность проектируемого кода колеблется в пределах 3-4, что указывает на умеренно низкий уровень сложности. Данное значение свидетельствует о том, что код достаточно структурирован и его логика легко воспринимается, что в свою очередь способствует лучшему пониманию и уменьшает вероятность возникновения ошибок.
Покрытие кода тестами составляет 32%, что указывает на необходимость увеличения объема автоматизированного тестирования. Это значение свидетельствует о том, что значительная часть функциональности остается неохваченной тестами, что может привести к потенциальным рискам в виде незамеченных ошибок и проблем с производительностью.
Поддерживаемость кода была оценена на уровне 72%. Этим показателем подтверждается, что код достаточно легок для изменений и дополнений, что особенно важно для долгосрочного сопровождения проекта и его адаптации к изменяющимся требованиям.
Степень связности кода составляет 0.25, что указывает на низкий уровень зависимости между модулями. Это свидетельствует о том, что изменения в одном модуле минимально повлияют на другие, что способствует более легкому модифицированию системы в будущем.
Избыточность кода находится на уровне 10%. Это значение показывает, что в коде присутствует некоторое количество дублирующегося кода, что следует оптимизировать для улучшения качества и удобства его сопровождения.
Таким образом, результаты оценки качества кода подчеркивают как сильные, так и слабые стороны разработанного программного средства, позволяя наметить пути для дальнейшего улучшения его качества.
...
1. Тест метода findAll (получение всех договоров)
@Test
@WithMockUser(roles = {"USER"})
void findAll_shouldReturnOk() throws Exception {
    Ordering ordering = new Ordering();
    OrderingDto dto = mockDto();

    when(orderingService.findAll()).thenReturn(List.of(ordering));
...
Описание: Этот тест проверяет метод получения всех договоров через GET /orderings. С помощью моков создаётся список договоров и конвертируется в DTO. Тест убеждается, что ответ приходит со статусом 200 OK, поле success равно true, статусный код соответствует успешной операции, в сообщении содержится "Success Find All", а в поле data есть хотя бы один договор. Таким образом проверяется, что контроллер корректно возвращает список всех договоров.

2. Тест метода find (получение договора по ID)
@Test
@WithMockUser(roles = {"USER"})
void find_shouldReturnOk() throws Exception {
    ...
    when(orderingService.find("1")).thenReturn(ordering);
    when(toDtoConverter.convert(ordering)).thenReturn(dto);
...
Описание: Этот тест проверяет метод получения конкретного договора по его идентификатору через GET /orderings/{id}. С помощью моков создаётся договор и конвертируется в DTO. Тест проверяет, что ответ имеет статус 200 OK, сообщение "Success Find" и что данные договора присутствуют в ответе. Таким образом обеспечивается корректная работа метода поиска договора по ID.

3. Тест метода save_user (создание договора страхователем)
@Test
@WithMockUser(roles = {"USER"})
void save_user_shouldReturnOk() throws Exception {
    Ordering ordering = new Ordering();
    OrderingDto dto = mockDto();

    when(orderingService.save("s1", "t1")).thenReturn(ordering);
    when(toDtoConverter.convert(ordering)).thenReturn(dto);
...
Описание: Этот тест проверяет создание нового договора пользователем через POST /orderings. Используются моки сервиса и конвертера DTO, чтобы имитировать успешное сохранение договора. Тест проверяет, что запрос выполняется успешно, возвращается статус 200 OK и сообщение "Success Save", что подтверждает корректное создание договора пользователем.

4. Тест метода save_manager (создание договора менеджером для страхователя)
@Test
@WithMockUser(roles = {"MANAGER"})
void save_manager_shouldReturnOk() throws Exception {
    Ordering ordering = new Ordering();
    OrderingDto dto = mockDto();

    when(orderingService.save("s1", "t1", "u1")).thenReturn(ordering);
    when(toDtoConverter.convert(ordering)).thenReturn(dto);

    mockMvc.perform(post("/orderings/user")
                    .param("subjectId", "s1")
                    .param("typeId", "t1")
                    .param("userId", "u1"))
            .andExpect(status().isOk())
...
Описание: Этот тест проверяет возможность менеджера создать договор для конкретного пользователя через POST /orderings/user. Моки имитируют успешное сохранение договора и конвертацию в DTO. Тест проверяет, что ответ приходит со статусом 200 OK и содержит сообщение "Success Save", что подтверждает корректную работу метода создания договора менеджером.

5. Тест метода payment (оплата договора менеджером)
@Test
...
void payment_shouldReturnOk() throws Exception {
    ...
    when(orderingService.payment("1", "2024-01-01", "2024-01-31")).thenReturn(ordering);
    when(toDtoConverter.convert(ordering)).thenReturn(dto);

    mockMvc.perform(patch("/orderings/1/payment")
                    .param("dateStart", "2024-01-01")
                    .param("dateEnd", "2024-01-31"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.message").value("Success Payment"));
}


Описание: Этот тест проверяет метод оплаты договора старшим менеджером через PATCH /orderings/{id}/payment. С помощью моков создаётся договор и конвертируется в DTO. Тест проверяет, что операция выполняется успешно, возвращается статус 200 OK и сообщение "Success Payment", что подтверждает корректную работу метода оплаты договора и правильное формирование ответа контроллера.

### Интеграционные тесты

1. Тест метода findAll (получение всех предметов)
@Test
...
void findAll_shouldReturnSubjects() throws Exception {
    mockMvc.perform(get("/subjects"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.message").value("Success Find All"))
            .andExpect(jsonPath("$.data[0].name").value("Test Subject"));
}


Описание: Этот тест проверяет работу метода GET /subjects, который возвращает список всех предметов. В тесте используется встроенный MockMvc для выполнения запроса к контроллеру, при этом предварительно в базе создаётся тестовый предмет. Проверяется, что ответ приходит со статусом 200 OK, в сообщении содержится "Success Find All", а в списке данных присутствует созданный предмет с ожидаемым именем. Таким образом тест подтверждает корректную работу метода получения всех предметов и правильное формирование ответа API.

2. Тест метода save (создание нового предмета)
@Test
@WithMockUser(authorities = {"USER"})
void save_shouldCreateSubject() throws Exception {
    String json = """
            {
              "name": "New Subject"
            }
    ...
    mockMvc.perform(post("/subjects")
            ...
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.message").value("Success save"))
            .andExpect(jsonPath("$.data.name").value("New Subject"));
}


Описание: Этот тест проверяет метод POST /subjects, который создаёт новый предмет. В тесте формируется JSON с именем нового предмета и выполняется POST-запрос к контроллеру. Проверяется, что ответ имеет статус 200 OK, сообщение содержит "Success save", а созданный предмет в ответе имеет имя "New Subject". Тест позволяет убедиться, что контроллер корректно обрабатывает создание предметов, сохраняет их в базе и возвращает правильный формат ответа.
...
Инструкция по развертыванию программного средства представляет собой документ, в котором описывается процесс подготовки среды и установки программного обеспечения для его последующего корректного функционирования. Она необходима для того, чтобы обеспечить единообразие и повторяемость развертывания, исключить возможные ошибки при установке, упростить ввод системы в эксплуатацию, а также облегчить ее дальнейшее сопровождение и масштабирование. Такая инструкция позволяет любому пользователю или специалисту выполнить развертывание по заранее определённому сценарию, не прибегая к дополнительным уточнениям.
Необходимые компоненты. Чтобы установить и запустить программное средство страхования рисков, связанных с использованием банковских платежных карточек и электронных кошельков для физических лиц необходимо наличие следующих компонентов:
– операционная система Windows версии 8 и выше;
– веб-браузер;
– сервер базы данных MySQL;
– среда разработки Inteleji IDEA;
– Node.js и npm.
Последовательность установки. Соблюдение установленной последовательности действий при развертывании особенно важно, поскольку каждый шаг опирается на результаты предыдущего. Нарушение порядка может вызвать ошибки настройки, проблемы с зависимостями и некорректную работу программы.
Для развертывания разработанного программного средства необходимо последовательно выполнить предусмотренные этапы, каждый из которых обеспечивает правильную подготовку среды и дальнейшую стабильную работу системы. Необходимо соблюсти следующую последовательность установки:
...
– установить и настроить программное средство.
Как завершающий элемент подготовки среды используется файл конфигурации, содержащий настройки доступа к необходимым сервисам. В частности, конфигурационный файл application.yml определяет параметры подключения к базе данных. Далее приведено его содержимое.
...
Далее в разделе spring.jpa определяются параметры функционирования уровня доступа к данным. Значение параметра «hibernate.ddl-auto» установлено как update, что позволяет автоматически синхронизировать структуру таблиц с описанными сущностями без удаления существующих данных. Параметр «show-sql» активирует вывод выполняемых SQL-запросов, что облегчает анализ взаимодействия программного средства с базой данных и способствует более эффективной отладке.
Завершающий раздел файла – server – содержит параметр «port», определяющий порт, на котором запускается встроенный веб-сервер. Значение 8080 используется как стандартный порт для локального развертывания, обеспечивая удобный доступ к приложению во время разработки.
В совокупности данные параметры формируют базовую конфигурацию программного средства, обеспечивая корректное подключение к базе данных, управление логикой загрузки файлов и настройку сетевого взаимодействия.
Запуск программного средства. Запуск программного средства является заключительным этапом процесса разработки и развертывания, в ходе которого подготовленная система переводится в рабочий режим. На этом этапе выполняется настройка всех необходимых компонентов, подключение к базе данных, конфигурация среды и активация серверной и клиентской частей. Именно здесь обеспечивается готовность разработанного программного средства к эксплуатации, правильное взаимодействие всех модулей и возможность полноценного использования функционала конечными пользователями.
Для запуска программного средства страхования рисков, связанных с использованием банковских платежных карточек и электронных кошельков необходимо последовательно выполнить следующие действия:
1 Создать базу данных в MySQL. Для начала требуется подключиться к серверу MySQL и создать новую рабочую базу данных. Это можно сделать, выполнив команду:

CREATE DATABASE sure

После создания база будет доступна для подключения серверной части приложения.
2 Запустить серверную часть программного средства в среде разработки IntelliJ IDEA. На данном этапе производится запуск backend-модуля, который обеспечивает логическую обработку данных, взаимодействие с базой и предоставление API.
3 Активировать клиентскую часть проекта. Во фронтенд-части запуск осуществляется стандартными средствами среды разработки. В IntelliJ IDEA для этого используется кнопка запуска, что позволяет инициировать работу интерфейса пользователя.
4 Открыть программное средство в браузере. После успешного старта обеих частей системы в терминале отображается ссылка, по которой можно перейти к пользовательскому интерфейсу. Приложение становится доступным по адресу: http://localhost:4200.
Доступ возможен из любого современного веб-браузера.
Такая последовательность действий обеспечивает корректный запуск всех компонентов разработанного программного средства и позволяет приступить к его полноценному использованию.
Проверка работоспособности программного средства. После успешного запуска программы следует перейти к проверке его работоспособности, чтобы убедиться в корректной работе всех компонентов системы.
Для проверки программного средства следует выполнить следующие действия:
1 Проверка работы базы данных. Необходимо убедиться, что сервер MySQL запущен и доступен для соединения. Без активного сервера базы данных приложение не сможет корректно обрабатывать данные.
2 Проверка состояния серверной и клиентской частей. Следует удостовериться, что обе части программного средства находятся в рабочем режиме и корректно выполняют свои функции.
3 Доступ к программному средству через браузер. С помощью любого современного веб-браузера необходимо перейти по адресу http://localhost:4200. При успешном запуске в окне браузера откроется главная страница приложения, что свидетельствует о готовности системы к использованию.
Таким образом, последовательное выполнение всех этапов проверки позволяет убедиться в стабильной работе программного средства и его полной готовности к эксплуатации.
